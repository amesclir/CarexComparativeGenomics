...
chr1blocks$PatternCode <- c(1,0,1,0,1,0) 
...
chr2blocks[2,4] <- "fisfus"
chr2blocks[3,4] <- "conserved"
chr2blocks[4,4] <- "complex"
chr2blocks

BCR[BCR$Chromosome=="chr2",]
colnames(chr2blocks)<- c("Chromosome", "Start", "End", "Pattern")
chr2blocks$PatternCode <- c(0,1,0,1)
#There is change in order in the complex pattern, but we are not going to consider it
...
chr5blocks$PatternCode <- c(0,2,0,0) 
#There is change in order in the complex pattern, but we are not going to consider it
chr5blocks$CBR <- c(0,2,0,0)
#there are two CBR areas but both are in the fisfus of the patterns and not in the change order
chr5blocks
...
chr9blocks$PatternCode <- c(1,0,1,0) 
...
chr10blocks[4,4] <- "fisfus"
chr10blocks[5,4] <- "conserved"
chr10blocks[6,4] <- "fisfus"
...
chr10blocks$PatternCode <- c(0,1,0,1,0,1,0) 
...
newrow <- c("chr11", 5751244, 6162157, "fisfus")
chr11blocks <- insertRow(chr11blocks, newrow, 5)
newrow <- c("chr11", 7481618, 10687339, "fisfus")
chr11blocks <- insertRow(chr11blocks, newrow, 7)
chr11blocks[1,4] <- "fisfus"
...
chr11blocks$PatternCode <- c(1,0,1,0,1,0,1,0) 
chr11blocks$CBR <- c(0,0,0,0,0,0,0,1)
...
chr12blocks$PatternCode <- c(0,2,0,1,0,1,0,1,0) 
chr12blocks$CBR <- c(0,0,0,0,0,0,0,0,0)
#the first BCR in after and out of the known conserved
chr12blocks
...
chr13blocks[1,4] <- "complex"
chr13blocks[2,4] <- "conserved"
chr13blocks[4,4] <- "conserved"
chr13blocks[5,4] <- "complex"
chr13blocks

BCR[BCR$Chromosome=="chr13",]
colnames(chr13blocks)<- c("Chromosome", "Start", "End", "Pattern")
chr13blocks$PatternCode <- c(3,0,1,0,2)
#the second complex is considered by 1 because the unsorting is smalll
...
chr18blocks[2,4] <- "complex"
chr18blocks[3,4] <- "conserved"
chr18blocks[4,4] <- "complex"
...
chr18blocks$PatternCode <- c(NA,2,0,2,0,1,0)
chr18blocks$CBR <- c(NA,1,0,0,0,0,1)
#We are not consider the maybe conserved as there is a CBR close 
chr18blocks
...
chr19blocks$PatternCode <- c(0,2,0,1,0)
...
newrow <- c("chr21", 5440874, 6958720, "fisfus")
chr21blocks <- insertRow(chr21blocks, newrow, 3)
chr21blocks[1,4] <- "fisfus"
...
#We are not going to consider the maybeconserved ones and fusion the fisfus
chr21blocks$PatternCode <- c(3, NA, NA, NA)
chr21blocks$CBR <- c(0,NA,NA,NA)
chr21blocks
...
chr25blocks$PatternCode <- c(NA, 1, 0, 2,0)
...
chr28blocks$PatternCode <- c(0, NA)
#we are not going to consider the unsorting
